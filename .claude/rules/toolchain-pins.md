...
| `aqua:open-policy-agent/conftest`, `aqua:vale-cli/vale`, `aqua:jqlang/jq` | `CONFTEST_VERSION`, `VALE_VERSION`, `JQ_VERSION` in `engineering-conventions/bin/conventions` | Consumers run the checks with the versions this repository tested them with (`tests/test_launcher.py` fails otherwise) |
